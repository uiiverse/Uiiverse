...
if ($reply_result->num_rows == 0) {
    exit('<title>Uiiverse - Error</title><div class="no-content track-error" data-track-error="404"><div><p>The reply could not be found.</p></div></div>');
}
...
echo '
    	<div class="main-column"><div class="post-list-outline">
    	  <a class="post-permalink-button info-ticker" href="/posts/'. $post['id'] .'">
    	    <span class="icon-container"><img src="'. printFace($post['user_face'], $post['feeling_id']) .'" class="icon"></span>
    	...
    	<div class="post-list-outline">
    	  <div id="post-content" class="post reply-permalink-post">
    	    <p class="community-container">
              ...
              <div id="user-content">
              <title>Uiiverse - '. htmlspecialchars($user['nickname'], ENT_QUOTES) .'\'s Comment</title>
        <a href="/users/'. $user['user_name'] .'/posts" class="icon-container'.($user['user_level'] > 1 ? ' verified' : '').'"><img src="'. printFace($user['user_face'], $reply['feeling_id']) .'" class="icon"></a>
...
if (!checkYeahAdded($reply['reply_id'], 'reply', $_SESSION['user_id'])) {
    echo '
    <a href="/users/'. $user['user_name'] .'/posts" class="icon-container'. ($user['user_level'] > 1 ? ' verified' : '') .' visitor" style="display: none;"><img src="'. printFace($user['user_face'], $reply['feeling_id']) .'" class="icon"></a>';
} else {
    echo '<a href="/users/'. $user['user_name'] .'/posts" class="icon-container'. ($user['user_level'] > 1 ? ' verified' : '') .' visitor">
    <img src="'. printFace($user['user_face'], $reply['feeling_id']) .'" class="icon"></a>';
}
...
while ($yeah_by = $yeahs_by_result->fetch_array()) {
    ...
    <img src="' . printFace($yeah_by['user_face'], $reply['feeling_id']) . '" class="icon">
    </a>';
}
