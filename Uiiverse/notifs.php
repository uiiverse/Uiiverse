...
if (isset($_COOKIE['retro-mode'])) {
    echo '<link rel="stylesheet" type="text/css" href="/assets/css/retro/news-my-news.css"> <link rel="stylesheet" type="text/css" href="/assets/css/retro/root.css">';
    }
printHeader(4);
    ...
    while ($notif = $notifs_result->fetch_array()) {
        ...
        echo '<div class="news-list-content'. ($notif['notif_read'] == 0 ? ' notify' : '') .' trigger" tabindex="0" data-href="'.$notifurl.'">
			<a href="/users/'. $user['user_name'] .'/posts" class="icon-container'. ($user['user_level'] > 1 && $notif['notif_type'] != 5 ? ' verified' : '') .'">
				<img src="'. ($notif['notif_type'] == 5 ? '/assets/img/uiiverse-administrator.png' : printFace($user['user_face'], 0)) .'" class="icon">
			</a>
			<div class="body">';
        ...
        if ($notif['notif_type'] == 5) {
            echo '
			<p class="title">
			  <span class="nick-name">Uiiverse Administration</span>
			  <span class="id-name">uiiverse_admin</span></p>
			<p class="text">You have received a notification from the Uiiverse administrators.';
        } else {
