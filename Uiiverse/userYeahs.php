    ...
    if (!$yeahs_result->num_rows == 0) {
        ...
        while ($yeahs = $yeahs_result->fetch_array()) {
            ...
            if ($yeahs['type'] == "post") {
                ...
                echo '"><img src="'. printFace($reply_user['user_face'], $replies['feeling_id']) .'" class="icon"></a><p class="user-name"><a href="/users/'. $reply_user['user_name'] .'/posts">'. htmlspecialchars($reply_user['nickname'], ENT_QUOTES) .'</a></p><p class="timestamp-container"><a id="timestamp">' .
